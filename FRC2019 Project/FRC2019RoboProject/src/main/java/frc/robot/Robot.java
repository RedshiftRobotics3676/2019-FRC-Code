...
import frc.robot.subsystems.LineTracking;
...
public class Robot extends TimedRobot {
  ...
  public static StringBuilder nya = new StringBuilder();
  ...
  public static AHRS knavX;
  public static LineTracking kLinetracking;
  ...
  public void robotInit() {
    ...
    //arm motors
    armMaster = new WPI_TalonSRX(RobotMap.armTalonID);
    armConsort = new WPI_VictorSPX(RobotMap.armVictorID);

    //elevator motors
    eTalon = new WPI_TalonSRX(RobotMap.elevatorTalonID);
    eVictor = new WPI_VictorSPX(RobotMap.elevatorVictorID);
    top = new DigitalInput(RobotMap.upSwitchPort);
    bottom = new DigitalInput(RobotMap.downSwitchPort);
    //hatch intake solenoid
    ...
    kArm = new Arm(armMaster, armConsort);
    kHatchIntake = new HatchIntake(hiDS);
    kIntake = new Intake(iVictor, touch);

        //navX
        knavX = new AHRS(SPI.Port.kMXP);


        //master Talons
        leftTalon = new WPI_TalonSRX(RobotMap.leftMasterID);
        rightTalon = new WPI_TalonSRX(RobotMap.rightMasterID);
        kDriveTrain = new DriveTrain(leftTalon, rightTalon, leftVictor, rightVictor);
        kArm = new Arm(armMaster, armConsort);
        kElevator = new Elevator(eTalon, eVictor, top, bottom);
        kHatchIntake = new HatchIntake(hiDS);
        kLinetracking = new LineTracking();
  ...
  public void teleopPeriodic() {
    ...
    //Scheduler.getInstance().add(new ArmUp());
    //Scheduler.getInstance().add(new VisionDriving());
